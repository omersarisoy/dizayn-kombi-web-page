
<footer>
    <div class="container-fluid up-footer">
        <div class="row">
            <div class="design-container">
                <div class="bulten-member ">
                    <h3>E-Bülten Aboneliği</h3>
                    <p>Eposta adresinizi kayıt ederek güncel gelişmeler ve koleksiyonlardan haberdar olabilirsiniz.</p>
                    <div>
                        <input type="email" class="form-control" placeholder="E-Posta Adresinizi Giriniz" id="exampleInputEmail1" aria-describedby="emailHelp">
                        <button>
                            Gönder <i class="fas fa-long-arrow-alt-right"></i>
                        </button>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="design-container main-footer">
        <div class="row">
            <div class="col-3">
                <img src="./image/dizayn-logo.png" alt="logo">
                <p>Atatürk Mah. Adnan Menderes Cad. No:6, 34522 Esenyurt İstanbul
                    info@dizaynkombi.com</p>
                <a href="facetime:14085551234"><i class="fas fa-phone"></i> +90 850 290 3434</a>
            </div>
            <div class="col-3">
                <h4>KURUMSAL</h4>
                <ul>
                    <li><a href="">Hakkımızda</a></li>
                    <li><a href="">Sıkça Sorulan Sorular</a></li>
                    <li><a href="">Tanıtım Filmi</a></li>
                </ul>
            </div>
            <div class="col-3">
                <h4>ÜRÜNLER</h4>
                <ul>
                    <li><a href="">Kombitech plus Kombi</a></li>
                    <li><a href="">Doru Kombi</a></li>
                    <li><a href="">Oda Termostati</a></li>
                </ul>
            </div>
            <div class="col-3">
                <h4>MÜŞTERİ HİZMETLERİ</h4>
                <ul>
                    <li><a href="">Servislerimiz</a></li>
                    <li><a href="">Görüşlerinize İhtiyacımız Var</a></li>
                    <li><a href="">Bayilik Başvuru Formu</a></li>
                </ul>
            </div>
        </div>
    </div>

    <div class=" design-container down-footer">
        <div>
            <p>Copyright © 2019 </p>
            <a href=""> Dizayn Grup. </a>
            <p>Tüm hakları saklıdır. </p>
        </div>
        <div>
            <img src="./image/footer-card.png" alt="cards">
        </div>
    </div>
</footer>
<script src="./js/jquery.js"></script>
<script src="./js/swiper.js"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous">
</script>
<script src="./js/custom.js"></script>

</body>

</html>